...
*/
var HierarchicalKey = function(bytes, currency) {
  if(!currency){
    currency = 'btc';
  }
  ...
  if (typeof bytes == 'undefined' || bytes == 'mainnet' || bytes == 'livenet') {
    bytes = 'livenet';
    this.version = networks[currency]['livenet'].hkeyPrivateVersion;
  } else if (bytes == 'testnet') {
    this.version = networks[currency]['testnet'].hkeyPrivateVersion;
  }
...
HierarchicalKey.seed = function(bytes, network, currency) {
  if (!network)
    network = 'livenet';
...
HierarchicalKey.prototype.initFromBytes = function(bytes) {
  ...
  var isPrivate =
    (this.version == networks[this.currency]['livenet'].hkeyPrivateVersion ||
      this.version == networks[this.currency]['testnet'].hkeyPrivateVersion);

  var isPublic =
    (this.version == networks[this.currency]['livenet'].hkeyPublicVersion ||
      this.version == networks[this.currency]['testnet'].hkeyPublicVersion);
...
HierarchicalKey.prototype.buildExtendedPublicKey = function() {
  ...
  var v = null;
  switch (this.version) {
    case networks[this.currency]['livenet'].hkeyPublicVersion:
    case networks[this.currency]['livenet'].hkeyPrivateVersion:
      v = networks[this.currency]['livenet'].hkeyPublicVersion;
      break;
    case networks[this.currency]['testnet'].hkeyPublicVersion:
    case networks[this.currency]['testnet'].hkeyPrivateVersion:
      v = networks[this.currency]['testnet'].hkeyPublicVersion;
      break;
    default:
...
HierarchicalKey.prototype.deriveChild = function(i) {
  ...
  if (this.hasPrivateKey) {
    ...
    ret = new HierarchicalKey(null, this.currency);
    ret.chainCode = ir;
