...
Verifier.checkCopayers = function(credentials, copayers) {
  ...
  _.each(copayers, function(copayer) {
    ...
    // Not signed pub keys
    if (!copayer.encryptedName || !copayer.xPubKey || !copayer.requestPubKey || !copayer.signature) {
      log.error('Missing copayer fields in server response');
      error = true;
    } else {
      var hash = Utils.getCopayerHash(copayer.encryptedName, copayer.xPubKey, copayer.requestPubKey);
