...
Verifier.checkTxProposal = function(credentials, txp, opts, cb) {
  ...
  if (!creatorKeys) return cb(null, true);
  ...
  if (txp.payProUrl && !opts.forAirGapped) {
    PayProRequest.get({
      url: txp.payProUrl,
      ...
      if ((txp.toAddress !== paypro.toAddress || txp.amount !== paypro.amount)) {
        log.debug('Proposal paramaters do not match PayPro Request')
        return cb(null, true);
      }
