...
Verifier.checkTxProposalBody = function(credentials, txp) {
  ...
  if (txp.outputs) {
    _.each(txp.outputs, function(o) {
      o.message = o.encryptedMessage || o.message;
      delete o.encryptedMessage;
    });
    var proposalHeader = {
      outputs: txp.outputs,
      message: txp.encryptedMessage || txp.message,
      // TODO: support payProUrl for multiple outputs
    };
    hash = WalletUtils.getProposalHash(proposalHeader);
  } else {
    hash = WalletUtils.getProposalHash(txp.toAddress, txp.amount, txp.encryptedMessage || txp.message, txp.payProUrl);
  }
