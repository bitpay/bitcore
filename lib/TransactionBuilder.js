...
function TransactionBuilder(opts) {
  ...
  this.headAddress      = opts.headAddress || '';
...
TransactionBuilder.infoForP2sh = function(opts, netname) {
  var script = this._scriptForOut(opts);
  var hash   = util.sha256ripe160(script.getBuffer());

  var network = networks[netname];
  if(!network) {
    throw new Error('network wrong ' + netname);
  }
  var version = network.P2SHVersion;
...
TransactionBuilder.prototype._multiFindKey = function(walletKeyMap,pubKeyHash) {
  var wk;
  for(var netname in networks) {
    var n = networks[netname];
    if(!n || !n.hasOwnProperty('addressVersion')) continue;
