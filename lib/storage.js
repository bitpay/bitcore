...
/*
 * This represent a current query stream from a Wallet client
 */
Storage.prototype.storeTxHistoryStreamV8 = function(walletId, since, limit, agent, items, cb) {
  var self = this;

  // only 1 per wallet is allowed
  self.db.collection(collections.CACHE).update({
    walletId: walletId,
    type: 'historyStreamV8',
    key: null
  }, {
    walletId: walletId,
    type: 'historyStreamV8',
...
Storage.prototype.getTxHistoryStreamV8 = function(walletId, since, limit, agent, cb) {
  var self = this;
  self.db.collection(collections.CACHE).findOne({
    walletId: walletId,
    type: 'historyStream',
