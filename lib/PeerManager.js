...
function PeerManager(config) {
  ...
  var defaultConfig = _.clone(bitcoreDefaults);
...
PeerManager.prototype.checkStatus = function checkStatus() {
  ...
  if(this.peers.length) {
    ...
    log.info(this.config.network, connected + ' of ' + this.peers.length + ' peers connected');
...
PeerManager.prototype.handleVersion = function(e) {
  ...
  if(this.peerDiscovery &&
      (e.message.version >= 31402 || this.peers.length < this.peerLimit)) {
    e.conn.sendGetAddr();
    e.conn.getaddr = true;
  }
};
...
PeerManager.prototype.handleAddr = function (e) {
  ...
  e.message.addrs.forEach(function (addr) {
    if(self.peers.length >= self.peerLimit) {
      return;
    }
    try {
