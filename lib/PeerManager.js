...
function PeerManager(config) {
  ...
  this.config = config || {};
  this.config.currency = this.config.currency || 'btc';
...
PeerManager.prototype.discover = function(options, callback) {
  var self = this;
  var seeds = networks[self.config.currency][self.config.network].dnsSeeds;
    ...
    return function(done) {
      ...
      dns.resolve(seed, function(err, peers) {
        ...
        peers = peers.map(function(ip) {
          return new Peer(ip, networks[self.config.currency][self.config.network].defaultClientPort);
        });
