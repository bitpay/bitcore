...
API.prototype._validateKeys = function() {
  ...
  var xpriv = new Bitcore.HDPrivateKey(c.xPrivKey);
  var priv = xpriv.derive(path).derive(nonHardenedPath).privateKey;
 ...
 * @param {String} opts.addressType - default 'P2PKH'
 */
API.prototype.seedFromExtendedPrivateKey = function(xPrivKey, opts) {
  opts = opts || {};
  this.credentials = Credentials.fromExtendedPrivateKey(xPrivKey, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts.addressType || Constants.SCRIPT_TYPES.P2PKH);
};
