...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testMessageSigning(xpriv, xpub) {
    ...
    var priv = xpriv.deriveNonCompliantChild(nonHardenedPath).privateKey;
  ...
  function testHardcodedKeys() {
    ...
    xpriv = xpriv.deriveNonCompliantChild("m/44'/0'/0'");
  ...
  function testLiveKeys() {
    ...
    xpriv = xpriv.deriveNonCompliantChild(c.getBaseAddressDerivationPath());
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  _.each(txp.inputs, function(i) {
    ...
    if (!derived[i.path]) {
      derived[i.path] = xpriv.deriveNonCompliantChild(i.path).privateKey;
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var xPriv = new Bitcore.HDPrivateKey(this.credentials.xPrivKey)
    .deriveNonCompliantChild(this.credentials.getBaseAddressDerivationPath());
