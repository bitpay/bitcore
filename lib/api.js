...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  self._doPostRequest('api/wallet', args, function(err, res) {
    if (err) return cb(err);
 ...
 * @param {Boolean} opts.twoStep[=false] - Optional: use 2-step balance computation for improved performance
 * @param {String} opts.walletId - Required: the wallet to lookup by id, TODO should be public key
 * @param {Callback} cb
 */
API.prototype.getBalance = function(opts, cb) {
  ...
  var url = 'api/wallet/' + opts.wallet + '/balance';
  this._doGetRequest(url, (err, resp) => {
   cb(err, {totalAmount: resp.balance, lockedAmount: -1});
  });
};
