...
API.prototype.createWalletFromOldCopay = function(username, password, blob, cb) {
  ...
  var seed = new Buffer(w.secretNumber);
  var privKey = new Bitcore.PrivateKey.fromBuffer(Bitcore.crypto.Hash.sha256(seed));

  var opts = {
    m: w.opts.requiredCopayers,
    n: w.opts.totalCopayers,
    name: w.opts.name,
    privateKeyHex: w.privateKey.extendedPrivateKeyString,
    network: w.opts.networkName,
    id: w.publicKeyRing.walletId,
    walletPrivKey: privKey,
  };
console.log('[api.js.237:opts:]',opts); //TODO

  this.credentials = Credentials.fromExtendedPrivateKey(opts.privateKeyHex);
