...
API.prototype._validateKeys = function() {
  ...
  function testLiveKeys() {
    ...
    var xpriv;
    if (words && (!c.mnemonicHasPassphrase || c.passphrase)) {
      var m = new Mnemonic(words);
      xpriv = m.toHDPrivateKey(c.passphrase, c.network);
    }
    if (!xpriv) {
  ...
  this.incorrectDerivation = !(c.isPrivKeyEncrypted() ? testHardcodedKeys() : testLiveKeys());
