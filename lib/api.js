...
API.prototype.importFromMnemonic = function(words, opts, cb) {
  log.info('Importing from 12 Words');

  var nrWords = words.split(' ').map(function(v){
    return _.trim(v);
  }).length;
  if (nrWords != 12)
    return cb(Errors.INVALID_BACKUP);

  var key = Credentials.keyFromMnemonic(words, opts.passphrase, opts.network);
...
API.prototype.importFromExtendedPrivateKey = function(xPrivKey, opts, cb) {
  ...
  log.info('Importing from Extended Private Key');
  this.credentials = Credentials.fromExtendedPrivateKey(xPrivKey);
  ...
  self._doGetRequest('/v1/wallets/', function(err, ret) {
    ...
    if (self.credentials.walletPrivKey) {
      if (!Verifier.checkCopayers(self.credentials, wallet.copayers)) {
        return cb(Errors.SERVERCOMPROMISED);
      }
    } else if (wallet.n == 1) {
