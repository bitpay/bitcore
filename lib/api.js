...
API.prototype.initialize = function(cb) {
  ...
  var self = this;
  self._initNotifications(cb);
};

API.prototype.dispose = function(cb) {
  var self = this;
  if (self.notificationsIntervalId) clearInterval(self.notificationsIntervalId);

  return cb();
};

API.prototype._initNotifications = function(cb) {
  var self = this;

  var NOTIFICATION_INTERVAL = 5; // in seconds

  var lastNotificationId;
  self.notificationsIntervalId = setInterval(function() {
    var url = '/v1/notifications/';
    if (lastNotificationId) url += '?notificationId=' + lastNotificationId;
    ...
    self.getNotifications({
      ...
      _.each(notifications, function(notification) {
        self.emit('notification', notification.data);
      });
    });
