...
API.prototype._validateKeys = function() {
  ...
  if (c.isPrivKeyEncrypted()) {
    xpriv = Bitcore.HDPrivateKey.fromString('xprv9s21ZrQH143K27TCUg7za7VcxPUuopCLRaw9Km6JDyCuAo91dGzaHtAZ3oqbUpnjnnwMtYBoGDrDpivNNhkwGuASsgPdaTUMyw8zLxk4QTQ');
    xpriv = xpriv.derive("m/44'/0'/0'");
    if (xpriv.toString() != 'xprv9y7eAAfUKfnC42MnP2dLwRxK5hafPJdcNVoP1Gyc8KogHks3558bemKfarc6EcXCmnoTVDFPFQe41oLHntKfTn9v712eJYJugBBT2zowxJL') {
      this.incorrectDerivation = true;
      return false;
    }
    xpub = Bitcore.HDPublicKey.fromString('xpub6C6zZgCNA3LVGWSFV4AMJZu3djR9nmMTjiiyofPDgfLfAZCBccSrCZe9S6yDbyKbu1Thpk612KCeDw9MBRatjv59saNzcKSoJcrLpALHZeB');
  } else {
    xpriv = new Bitcore.HDPrivateKey(c.xPrivKey).derive(path);
    xpub = new Bitcore.HDPublicKey(c.xPubKey);
  }
  ...
  this.incorrectDerivation = !Utils.verifyMessage(message, signature, pub);
