...
API.prototype.createWalletFromOldCopay = function(username, password, blob, cb) {
  ...
  if (!decrypted)
    return cb('Could not decrypt');

  var w = JSON.parse(decrypted);
  ...
  var opts = {
    m: w.opts.requiredCopayers,
    n: w.opts.totalCopayers,
    name: w.opts.name,
    xpriv: w.privateKey.extendedPrivateKeyString,
    network: w.opts.networkName,
    id: w.opts.id,
    walletPrivKey: privKey.toString(),
  };
  this.credentials = Credentials.fromExtendedPrivateKey(opts.xpriv);

  var self = this;
  var pkr = _.map(w.publicKeyRing.copayersExtPubKeys, function(xPubStr) {

    var isMe = xPubStr === self.credentials.xPubKey;
    var requestDerivation;

    if (isMe) {
      var path = WalletUtils.PATHS.REQUEST_KEY;
      requestDerivation = (new Bitcore.HDPrivateKey(self.credentials.xPrivKey))
  ...
  this.credentials.addPublicKeyRing(pkr);

  this.createWallet(opts.name, username, opts.m, opts.n, {
    network: opts.network,
    id: opts.id,
    walletPrivKey: opts.walletPrivKey,
  }, function(err, secret) {
    if (err && err.message == 'Wallet already exists') {
      self.credentials.addWalletInfo(opts.id, opts.name, opts.m, opts.n,
        opts.walletPrivKey, username);
      return self._replaceTemporaryRequestKey(cb);
    }
    ...
    async.eachSeries(self.credentials.publicKeyRing, function(item, next) {
      ...
      i++;
      self._doJoinWallet(opts.id, opts.walletPrivKey, item.xPubKey, item.requestPubKey, copayerName, {
...
API.prototype.openWallet = function(cb) {
  ...
  if (self.credentials.isComplete() && !self.credentials.hasTemporaryRequestKeys())
    return cb(null, true);
  ...
  self._doGetRequest('/v1/wallets/', function(err, ret) {
    ...
    if (self.credentials.walletPrivKey) {
      ...
      log.warn('Could not verify copayers key (missing wallet Privave Key');
    }

    if (self.credentials.isComplete()) {
