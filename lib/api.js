...
API.signTxProposalFromAirGapped = function(key, txp, unencryptedPkr, m, n, network, passphrase, account, derivationStrategy) {
  ...
  if (key.slice(0, 4) === 'xprv' || key.slice(0, 4) === 'tprv') {
    if (key.slice(0, 4) === 'xprv' && network == 'testnet') throw new Error("testnet HD keys must start with tprv");
    if (key.slice(0, 4) === 'tprv' && network == 'livenet') throw new Error("livenet HD keys must start with xprv");
