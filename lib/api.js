...
API.prototype._processStatus = function(status) {
  ...
  function processWallet(wallet) {
    ...
    wallet.name = Utils.decryptMessage(wallet.name, encryptingKey);
    _.each(wallet.copayers, function(copayer) {
      copayer.encryptedName = copayer.name;
      copayer.name = Utils.decryptMessage(copayer.name, encryptingKey);
      _.each(copayer.requestPubKeys, function(access) {
        if (!access.name) return;
        access.encryptedName = access.name;
        access.name = Utils.decryptMessage(access.name, encryptingKey);
      });
    });
  };
