...
API.prototype._processTxps = function(txps) {
  ...
  _.each([].concat(txps), function(txp) {
    if (parseInt(txp.version) < 3) txp.encryptedMessage = txp.message;
    ...
    _.each(txp.outputs, function(output) {
      if (parseInt(txp.version) < 3) output.encryptedMessage = output.message;
 ...
 * @param {String} opts.outputs[].toAddress
 ...
 *
 * @param {Object} opts
 * @param {Object} opts.txpId
 * @returns {Callback} cb - Return error or null
 */
API.prototype.publishTxProposal = function(opts, cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  $.checkArgument(opts)
    .checkArgument(opts.txpId);

  var self = this;

  self.getTx(opts.txpId, function(err, txp) {
    if (err) return cb(err);
    var t = Utils.buildTx(txp);
    var message = t.uncheckedSerialize();
    var args = {
      proposalSignature: Utils.signMessage(message, self.credentials.requestPrivKey)
    };

    var url = '/v1/txproposals/' + txp.id + '/publish/';

    self._doPostRequest(url, args, function(err) {
      if (err) return cb(err);
      return cb();
    });
  });
