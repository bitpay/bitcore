 ...
 * @param {String} entropySourceHex -  an HEX string containing random data, that can be reproducible by the device
 */
...
API.prototype.importFromMnemonic = function(words, opts, cb) {
  ...
  var key;
  try {
    key = Credentials.keyFromMnemonic(words, opts.passphrase, opts.network);
  } catch (e) {
    log.info('Mnemonic error:', e);
    return cb(Errors.INVALID_BACKUP);
  };

  return this.importFromExtendedPrivateKey(key, opts, cb);
};
...
API.prototype.importFromExtendedPrivateKey = function(xPrivKey, opts, cb) {
  ...
  self.openWallet(function(err, ret) {
    // it worked?
    // TODO download encrypted Wallet key ...
    ...
    self.addAccess({}, function(err) {
      ...
      log.info('Could not add access... trying to recreate wallet');
      if (!self.credentials.network || !opts.m || !opts.n)
        return cb(Errors.MISSING_ARGS_TO_RECOVER_WALLET);

      if (opts.n > 1 && !opts.publicKeyRing)
        return cb(Errors.MISSING_ARGS_TO_RECOVER_WALLET);

      self.credentials.m = opts.m;
      self.credentials.n = opts.n;

      // TODO: n>1 ?
      self.credentials.walletPrivKey = (new Bitcore.PrivateKey()).toString();

      if (opts.n > 1) {
        self.credentials.publicKeyRing = _.union(self.credentials.publicKeyRing, opts.publicKeyRing);
      }

      // Third option: create the wallet
      self.recreateWallet(cb);
    });
  });
};
...
API.prototype.openWallet = function(cb) {
  ...
  self._doGetRequest('/v2/wallets/?includeExtendedInfo=1', function(err, ret) {
    ...
    if (self.credentials.walletPrivKey) {
      ...
      if (!Verifier.checkOneCopayer(self.credentials, wallet.copayers)) {
        return cb(Errors.SERVER_COMPROMISED);
      }
    } else {
...
API.prototype.recreateWallet = function(cb) {
  ...
  self._doPostRequest('/v1/wallets/', args, function(err, body) {
    ...
    async.each(self.credentials.publicKeyRing, function(item, next) {
      ...
      self._doJoinWallet(walletId, walletPrivKey, item.xPubKey, item.requestPubKey, name, {}, function(err) {
...
API.prototype._processCustomData = function(result) {
  ...
  try {
    result.customData = JSON.parse(WalletUtils.decryptMessage(me.customData, this.credentials.personalEncryptingKey));
  } catch (e) {
    log.info('Could not decrypt customData:', result.customData);
  }

  if (this.credentials.sharedEncryptingKey) return;

  if (result.customData.walletPrivKey)
    this.credentials.addWalletPrivateKey(result.customData.walletPrivKey)
}
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var xPriv = new Bitcore.HDPrivateKey(this.credentials.xPrivKey)
    .derive(WalletUtils.PATHS.BASE_ADDRESS_DERIVATION);
