...
API._getBaseAddressDerivationPath = function(derivationStrategy, network, account) {
...
API._deriveXPrivFromMaster = function(masterXPriv, derivationStrategy, network, account) {
  var path = API._getBaseAddressDerivationPath(derivationStrategy, network, account || 0);
...
API.prototype._signTxp = function(txp) {
  ...
  var xpriv = API._deriveXPrivFromMaster(self.credentials.xPrivKey, self.credentials.derivationStrategy, txp.network);
...
API.prototype.recreateWallet = function(cb) {
  ...
  this.getStatus({
    ...
    var supportBIP44AndP2PKH = self.credentials.derivationStrategy == Constants.DERIVATION_STRATEGIES.BIP44;
