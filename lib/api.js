...
API.prototype._processStatus = function(status) {
  ...
  function processWallet(wallet) {
    _.each(wallet.copayers, function(copayer) {
      ...
      copayer.name = Utils.decryptMessage(copayer.name, self.credentials.sharedEncryptingKey)
      _.each(copayer.requestPubKeys, function(key) {
        if (!key.name) return;
        key.encryptedName = key.name;
        key.name = Utils.decryptMessage(key.name, self.credentials.sharedEncryptingKey)
      });
    });
  };
