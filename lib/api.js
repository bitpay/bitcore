...
API.prototype._getCreateTxProposalArgs = function(opts, cb) {
  var self = this;

  var changeAddress, args;
  async.series([

    function(next) {
      if (!self.credentials.noPrivacy) return next();
      self.createAddress({}, function(err, address) {
        if (err) return next(err);
        if (!address) return next(new Error('Could not get change address'));
    ...
    function(next) {
      args = {
        message: API._encryptMessage(opts.message, self.credentials.sharedEncryptingKey) || null,
        fee: opts.fee,
        feePerKb: opts.feePerKb,
        changeAddress: changeAddress || opts.changeAddress,
        payProUrl: opts.payProUrl || null,
        excludeUnconfirmedUtxos: !!opts.excludeUnconfirmedUtxos,
        customData: opts.customData,
        inputs: opts.inputs,
...
API.prototype.createTxProposal = function(opts, cb) {
  ...
  self._getCreateTxProposalArgs(opts, function(err, args) {
    if (err) return cb(err);
...
API.prototype.createAddress = function(opts, cb) {
  ...
  function getNewAddress(done) {
    self._doPostRequest('/v3/addresses/', opts, done);
  };

  function getFirstAddress(done) {
    self.getMainAddresses({
      ...
      if (!_.isEmpty(addresses)) return done(null, _.first(addresses));
      return getNewAddress(done);
    });
  };
