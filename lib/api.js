 ...
 */
API.prototype.seedFromExtendedPublicKey = function(xPubKey, source, entropySourceHex, dummy) {
  $.checkArgument(!dummy, "DEPRECATED: seedFromExtendedPublicKey should receive only 3 parameters");
...
API.prototype.importFromExtendedPublicKey = function(xPubKey, source, entropySourceHex, cb, dummy) {
  $.checkArgument(!dummy, "DEPRECATED: importFromExtendedPublicKey should receive only 4 parameters");
  log.debug('Importing from Extended Private Key');
  try {
    this.credentials = Credentials.fromExtendedPublicKey(xPubKey, source, entropySourceHex);
  } catch (e) {
