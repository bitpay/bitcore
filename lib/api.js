...
API.prototype.decryptBIP38PrivateKey = function(encryptedPrivateKey, passphrase, opts, cb) {
  var Bip38 = require('bip38');
  var bip38 = new Bip38();

  var privateKey;
  try {
    var privateKeyWif = bip38.decrypt(encryptedPrivateKey, passphrase);
  ...
  var addrBuff = new Buffer(address.toString(), 'ascii');
  var actualChecksum = Bitcore.crypto.Hash.sha256sha256(addrBuff).toString('hex').substring(0, 8);
  var expectedChecksum = Bitcore.encoding.Base58Check.decode(encryptedPrivateKey).toString('hex').substring(6, 14);
