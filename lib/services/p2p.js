...
P2pService.prototype.sync = function(done) {
  ...
  Block.getLocalTip({chain: self.chain, network: self.network}, function(err, bestBlock) {
    if(bestBlock.height === self.getPoolHeight()) {
      logger.verbose('Already synced', {chain: self.chain, network: self.network, height: bestBlock.height});
      self.syncing = false;
      return done();
    }
    logger.info(`Syncing from ${bestBlock.height} to ${self.getPoolHeight()} for chain ${this.chain}`);
    let blockCounter = 0;
    async.during(
      function(cb) {
