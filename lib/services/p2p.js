...
P2pService.prototype.connect = function(ready){
  ...
  this.peer.on('open', function() {
    self.peer.on('packet', function(msg) {
      if(msg.cmd === 'block') {
...
P2pService.prototype.sync = function(done) {
  ...
  Block.getLocalTip(function(err, bestBlock) {
    ...
    self.syncing = true;
    var counter = 0;
    var bar = new ProgressBar('syncing [:bar] :percent :blockRate s/block :blocksEta hrs :blockTime', {
    ...
    async.during(
      ...
      function(cb) {
        async.eachOfSeries(self.headersQueue, function(header, index, cb) {
          if (index < self.headersQueue.length - 3){
            self.getBlock(self.headersQueue[index + 2]._hhash, function(err, block) {
              if(!err && block) {
                self.blockCache[self.headersQueue[index + 2]._hhash] = block;
              }
            });
          }
          var start = Date.now();
          self.getBlock(header._hhash, function(err, block) {
            Block.addBlock(block, function(err) {
              delete self.blockCache[header._hhash];
              var end = Date.now();
              counter++;
              ...
              bar.tick({
                ...
                blocksEta: ((((self.getPeerHeight() - bestBlock.height) - counter) * avgBlockRate) / 3600).toFixed(2)
              });
              cb(err);
            });
          });
...
P2pService.prototype._getHeaders = function(candidateHashes, callback) {
  candidateHashes = candidateHashes && candidateHashes.map(function(candidateHash){
...
P2pService.prototype.getBlock = function(hash, callback) {
  var self = this;
  if (self.blockCache[hash]){
    return callback(null, self.blockCache[hash]);
  }
