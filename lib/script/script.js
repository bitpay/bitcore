...
Script.prototype.toAddress = function(network) {
  ...
  var addressNetwork = Networks.get(network) || this._network || Networks.defaultNetwork;
  if (!info) {
    // Finally check for a public key output
    if (this.isPublicKeyOut()) {
      var publicKey = PublicKey.fromBuffer(this.chunks[0].buf);
      return Address.fromPublicKey(publicKey, addressNetwork);
    }
    return false;
  }
  info.network = addressNetwork;
