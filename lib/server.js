...
WalletService.prototype.syncWallet = function(wallet, cb) {
  ...
  self.registerWalletV8(wallet, (err) => {
    ...
    self.storage.fetchUnsyncAddresses(self.walletId, (err, addresses) => {
      ...
      self.logi('Syncing addresses: ', chunks.length); 
...
console.log('[server.js.3390:startBlock:]',startBlock); //TODO
console.log('[server.js.3390:bcHeight:]',bcHeight); //TODO
   var self = this;

   var all = [];
  startBlock = startBlock || 0;
  console.log(' ########### GET HISTORY! startBlock/bcH]',startBlock,bcHeight); //TODO

   // first mempool
  // then from bcHeight down to startBlock
  //
  var a= -1;
  var b= 0;
...
console.log('[server.js.3408]', a,b); //TODO
        bc.getTransactions(wallet, a, b, (err, txs) => {
          if (err)  {
            return next(err);
          }
...
console.log('[server.js.3483:streamKey:]',streamKey); //TODO
    ...
    if (err) return cb(err);
    return cb(null, { items: resultTxs, fromCache: fromCache, fromBc: fromBc } );
  });
};
    ...
    async.series([
      ...
      function(next) {
        ...
        _.each(txs, function(tx) {
          if (tx.height >= 0) {
            tx.confirmations = bcHeight - tx.height + 1;
          }
        });
        next();
      },
