...
WalletService.prototype._doGetTxHistoryV8 = function(bc, wallet, startBlock, bcHeight, cb) {
  ...
  console.log('[server.js.3374:startHeight:]',startBlock); //TODO
  ...
  self._runLocked(cb, function(cb) {
    var endBlock;
    endBlock = startBlock + pageSize;
    async.doWhilst(
      (next) => {
console.log('### [server.js.3389: start -> end ',startBlock, endBlock); //TODO
        bc.getTransactions(wallet, startBlock, endBlock, (err, txs) => {
          if (err)  {
            return next(err);
          }
      ...
      () => {
        if (endBlock == 0 ) {
          // mempool scanned
          return false;
        } else if ( endBlock < bcHeight )  {
          startBlock += pageSize;
          endBlock = startBlock + pageSize;
          return true;
        } else {
          // confirmed finished.. now:mempool
          startBlock = -1;
          endBlock  = 0;
          return true;
        }
