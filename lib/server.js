...
console.log('[server.js.1154:onlyMain:]',onlyMain); //TODO
...
//console.log('[server.js.2915:txs:] IN NORMALIZE',txs); //TODO
  ...
  var ret =  _.map([].concat(txs), function(tx) {
    ...
    switch (tx.category) {
      case 'send':
        ret.action = 'sent';
        ret.amount= Math.abs(_.sum(tx.outputs,'amount'));
...
console.log('[server.js.3171:addresses:]',addresses); //TODO
      if (err) {
        return cb(err);
      }
...
WalletService.prototype._doGetTxHistoryV8 = function(bc, wallet, startHeight, bcHeight, cb) {
  var self = this;

  var all = [];
  var pageSize = Defaults.FIRST_V8_TX_PAGE_SIZE;

  startHeight = startHeight || 0;
  console.log('[server.js.3374:startHeight:]',startHeight); //TODO

  if (!startHeight) {
    pageSize = 500000; //TODO
  } else {
    pageSize = 6 * 24; //TODO
  }
  ...
  self._runLocked(cb, function(cb) {
    ...
    async.doWhilst(
      (next) => {
        endBlock = startHeight + pageSize;
        bc.getTransactions(wallet, startHeight, endBlock, (err, txs) => {
console.log('[server.js.3380:txs:] RAW BC TXS',txs); //TODO
          if (err)  {
            done = true;
            return next(err);
          }
          ...
          startHeight += pageSize;
          return next();
        });
      },
      () => {
console.log('[server.js.3389:endBlock:/bcheight/ pagesize',endBlock, bcHeight, pageSize); //TODO
        return endBlock < bcHeight;
      },
...
WalletService.prototype.getTxHistoryV8 = function(bc, wallet, opts, skip, limit, cb) {
  ...
  async.series([
    ...
    (next) => {
      ...
      // update cache
      self._doGetTxHistoryV8(bc, wallet, cacheStatus.tipHeight, bcHeight, (err, inTxs) => {
        if (err) return cb(err);
