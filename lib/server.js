...
WalletService.prototype._runScan = function(wallet, opts, cb) {
  var self = this;
  ...
  function scanBranch(wallet, derivator, cb) {
    ...
    async.whilst(function() {
      self.logi('Scanning addr branch: %s index: %d gap %d step %d', derivator.id, derivator.index(), inactiveCounter, opts.step);
