...
function Connection(socket, peer, opts) {
  ...
  this.currency = this.config.currency || 'btc';
  
  this.network = networks[this.currency][this.config.network] || networks[this.currency].livenet;
  this.protocolVersion = this.network.peerVersion || PROTOCOL_VERSION;
  this.socket = socket;
  this.peer = peer;
  ...
  try {
    switch (message.command) {
      case 'version':
        ...
        this.sendVer = Math.min(message.version, this.protocolVersion);
        if (message.version < 209) {
          this.recvVer = Math.min(message.version, this.protocolVersion);
        } else {
...
Connection.prototype.sendVersion = function() {
  var subversion = '/' + networks[this.currency].name + 'X:0.1/';
  var put = new Put();
  put.word32le(this.protocolVersion); // version
...
Connection.prototype.parseMessage = function(command, payload) {
  ...
  switch (command) {
    ...
    case 'dsee': 
        log.debug("dsee")
        break;
    case 'dmcv'://DarkSend Masternode Consessus Vote
        log.debug("dmcv")
        break;
    case 'checkpoint':
