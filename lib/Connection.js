...
function Connection(socket, peer, opts) {
  ...
  this.network = networks[this.config.network] || networks.bitcoin;
  this.socket = socket;
  this.peer = peer;
  ...
  // post raw block message
  this.rawBlockMode = false;
...
Connection.prototype.sendRawInv = function (invs) {
  var put = new Put();
  put.varint(invs.length);
  for (var i = 0; i < invs.length; i++) {
    put.word32le(invs[i].type);
...
/*    log.err('Peer '+this.peer+' exceeded maxreceivebuffer, disconnecting.'+
                  (err.stack ? err.stack : err.toString())); */
    log.err('Peer exceeded maxreceivebuffer, disconnecting...');
    this.socket.destroy();
    return;
  }
...
Connection.prototype.parseMessage = function (command, payload) {
  ...
  case 'block':
    if(this.rawBlockMode) {
      data.command = 'rawblock';
      data.parser = parser;
    } else {
      var block = new Block();
      block.parse(parser);
