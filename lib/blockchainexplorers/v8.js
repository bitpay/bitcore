...
V8.prototype._doV8Request = async function(args,  authKey, cb) {
  request = args.request || require('request');

  const url = this.hosts[0] + args.path;
  const signature = this.sign(authKey, { method: args.method, url, payload: args.json });
  console.log('[v8.js.96:signature:]',signature); //TODO
  ...
  request(args, function(err, res, body) {
    if (err) {
      log.warn('V8 REQUEST FAIL: ERROR: ' + err);
    }

    if (res) {
      success = !!res.statusCode.toString().match(/^[1234]../);
      if (!success) {
        log.warn('REQUEST FAIL: STATUS CODE: ' + res.statusCode);
      }
    }
...
V8.prototype._doRequest = function(args, cb) {
...
V8.prototype.register = async function (name, xPubKey) {
 ...
 */
V8.prototype.getUtxos = function(addresses, cb, walletId) {
  var self = this; 
