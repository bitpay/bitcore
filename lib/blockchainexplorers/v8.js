...
V8.prototype.addAddresses = function (wallet, addresses, cb) {
  ...
  console.log('[v8.js.125:payload:]',payload); //TODO
...
V8.prototype.getUtxos = function(wallet, cb) {
  var self = this;
  var client = this._getAuthClient(wallet);
console.log('[v8.js.180:getUtxos v8:] wallet:', wallet.beAuthPublicKey);
...
console.log('[v8.js.184:unspent:]',unspent.length); //TODO
      _.each(unspent, function(x) {
        ...
        x.confirmations = x.confimations;
      });
...
V8.prototype.getTransactions = function(wallet, startBlock, endBlock , cb) {
console.time('V8 getTxs');
  if (startBlock) {
    log.debug(`getTxs: ${startBlock} => ${endBlock}`);
  } else {
    log.debug(`getTxs: complete`);
  }
   var self = this;
  ...
  if (_.isNumber(endBlock))
    opts.endBlock = endBlock;

console.log('[v8.js.284:opts:]',opts); //TODO
...
V8.prototype.initSocket = function(callbacks) {
  ...
  socket.on('coin', (data) => {
    ...
    var out = { 
      address: BCHAddressTranslator.translate(data.address, 'copay', 'cashaddr'),
