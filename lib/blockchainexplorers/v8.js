  ...
  client.getCoins({pubKey: wallet.beAuthPublicKey, payload: {} })
    .then( (unspent) => {
      _.each(unspent, function(x) {
        ...
        x.scriptPubKey = (Bitcore_[self.coin].Script.fromASM(x.script.asm)).toHex();
...
console.log('[v8.js.207] GET TX', txid); //TODO
  ...
  client.getTx({txid: txid })
    .then( (tx) => {
      ...
      // TODO: temporary fix v8 non-errors
      if (!tx || _.isEmpty(tx)) {
...
console.log('[v8.js.253:tx:]',tx); //TODO
    txs.push(tx);
  });
