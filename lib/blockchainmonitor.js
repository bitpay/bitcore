...
// TODO
if (pair.coin == 'btc') return;
if (pair.network == 'livenet') return;
...
BlockchainMonitor.prototype._initExplorer = function(coin, network, explorer) {
  ...
  socket.on('connect', function() {
    ...
    if (explorer.supportsGrouping()){
      socket.emit('room', '/' + coin.toUpperCase() +'/'  + explorer.v8network + '/inv');
    } else {
...
BlockchainMonitor.prototype._handleIncomingTx = function(coin, network, data) {
  log.info('New ' + network + ' tx: ' + data , JSON.stringify(data));
...
console.log('[blockchainmonitor.js.309:hash:]',hash); //TODO
console.log('[blockchainmonitor.js.309:network:]',network); //TODO
console.log('[blockchainmonitor.js.309:coin:]',coin); //TODO
