...
/*  if(this.auxPow) {
    return this.auxPow.blockHeader;
  } */
...
Block.prototype.parseMerkleBranch = function(parser) {
  var len = parser.varInt();
  var branch = {hashes: []};

  for(var i = 0; i< len; i++) {
    var hash = parser.buffer(32);
    branch.hashes.push(hash);
  }
  branch.side_mask = parser.word32le();
  return branch;
}
...
Block.prototype.parse = function parse(parser, headerOnly) {
  ...
  // FIXME: write 0x62 to network configs, currently only dogecoin have AuxPoW
  if(this.version >> 16 == 0x62) {
    // AuxPoW
    var auxPow = {};
    auxPow.coinbaseTx = new Transaction();
    auxPow.coinbaseTx.parse(parser);
    auxPow.block_hash = parser.buffer(32);
    auxPow.coinbaseBranch = this.parseMerkleBranch(parser);
    auxPow.blockchainBranch = this.parseMerkleBranch(parser);
    auxPow.blockHeader = parser.buffer(80);
    this.auxPow = auxPow;
  }
...
Block.prototype.calcHash = function calcHash(hashFunc) {
