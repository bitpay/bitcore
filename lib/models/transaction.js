...
TransactionSchema.statics.mintCoins = function (params) {
  return new Promise(async (resolve) => {
    ...
    let mintOps = [];
    for (let txIndex = txs.length - 1; txIndex>=0; txIndex--) {
      let txid = txs[txIndex].hash;
      let isCoinbase = txs[txIndex].isCoinbase();
      for (let outputIndex = txs[txIndex].outputs.length - 1; outputIndex >= 0; outputIndex--) {
        let output = txs[txIndex].outputs[outputIndex];
        let address;
