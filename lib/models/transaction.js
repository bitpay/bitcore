...
TransactionSchema.index({'outputs.wallets': 1, mainChain: 1, 'outputs.spent': 1}, {sparse: true});
...
TransactionSchema.statics.syncTransactionAndOutputs = function(data, callback) {
  ...
  Transaction.find({txid: transaction.rhash()}).limit(1).lean().exec(function(err, hasTx) {
    if(err) {
      return callback(err);
    }
    hasTx = hasTx[0];
...
TransactionSchema.statics.syncTransactionInputs = function(txid, callback) {
  Transaction.find({txid: txid}).limit().exec(function(err, transaction) {
    if(err) {
      return callback(err);
    }
    ...
    async.eachLimit(transaction.inputs, config.maxPoolSize, function(input, inputCb) {
      Transaction.find({txid: input.utxo}).limit(1).select('outputs').lean().exec(function(err, utxo) {
        if(err) {
          return inputCb(err);
        }
        utxo = utxo[0];
        ...
        spentUpdates.push({
          updateOne: {
            filter: {txid: input.utxo, 'outputs.vout': input.vout},
            update: {
      ...
      Transaction.bulkWrite(spentUpdates, {ordered:false}, function(err){
        if (err){
          return callback(err);
        }
