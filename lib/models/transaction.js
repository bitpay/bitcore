  ...
  async.series([
    function(cb){
      Transaction.mintCoins(params, async (err, mintOps) => {
        ...
        async.eachLimit(mintOps, 2, (mintBatch, cb) => {
          Coin.collection.bulkWrite(mintBatch, { ordered: false }, cb);
        }, cb);
      }, cb);
    },
    function(cb){
      ...
      async.eachLimit(spendOps, 2, (spendBatch, cb) => {
        Coin.collection.bulkWrite(spendBatch, { ordered: false }, cb);
      }, cb);
    },
    async function(cb){
      Transaction.addTransactions(params, (err, txOps) => {
        txOps = partition(txOps, 500);
        async.eachLimit(txOps, 2, (txBatch, cb) => {
          Transaction.collection.bulkWrite(txBatch, { ordered: false }, cb);
        }, cb);
      }, cb);
    }
  ], callback);
