...
TransactionSchema.statics.mintCoins = async function (params) {
  ...
  let mintOps = [];
  return new Promise((resolve, reject) => {
    async.eachLimit(txs, 20, (tx, cb) => {
      async.eachOfLimit(tx.outputs, 10, async function(output, index) {
        let address;
        ...
        let wallets = await WalletAddress.collection.find({address}).toArray();
