...
TransactionSchema.statics.addTransaction = async function(params, callback){
  let txid = params.transaction.hash;
  ...
  Transaction.findOneAndUpdate({ txid: txid}, {
    txid: txid,
    ...
    size: params.transaction.toBuffer().length,
    locktime: params.transaction.nLockTime,
    wallets: wallets
...
TransactionSchema.statics.mintCoins = function(params, callback){
  async.eachOfLimit(params.transaction.outputs, config.maxPoolSize, async function (output, index, outputCb) {
    let address;
    try {
      address = output.script.toAddress(params.network).toString();
      if (address === 'false' && output.script.classify() === 'Pay to public key') {
        ...
        address = bitcore.Address(hash, params.network).toString();
      }
    } catch (e) {
      address = 'noAddress';
    }
    ...
    Coin.findOneAndUpdate({ mintTxid: params.transaction.hash, mintIndex: index},{
      network: params.network,
      mintHeight: params.mintHeight,
      coinbase: params.transaction.isCoinbase(),
      value: output.satoshis,
      address: address,
      script: output.script && output.script.toHex(),
      spentHeight: -2,
      wallets: wallets
  ...
  async.eachLimit(params.transaction.inputs, config.maxPoolSize, function (input, inputCb) {
    ...
    Coin.findOneAndUpdate({
      ...
      mintIndex: input.outputIndex
        ...
        spentTxid: params.transaction.hash,
