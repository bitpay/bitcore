...
BlockSchema.statics.getLocalTip = function(params, callback) {
  const {chain, network} = params;
  Block.find({processed: true, chain, network}).sort({height: -1}).limit(1).exec(function(err, bestBlock) {
    if(err) {
      return callback(err);
    }
...
BlockSchema.statics.handleReorg = function(params, callback) {
  const { header, chain, network } = params;
  Block.getLocalTip(params, function(err, localTip) {
    if(err) {
      return callback(err);
    }
