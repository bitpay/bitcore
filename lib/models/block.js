  ...
  async.series([
    function(cb){
      Block.findOne({
        ...
        Block.update({mainChain: true, network: config.network, height: {$gte: existingBlock.height}},
          {$set: {mainChain: false}}, {multi: true}, function(err) {
            if(err){
              return cb(err);
            }
            // TODO need to also set all transactions with heights greater in this chain
