...
BlockSchema.statics.addBlock = function(block, callback){
  ...
  async.series([
    ...
    function(cb){
      async.mapLimit(block.txs, 10, function(transaction, txCb){
    ...
    function(cb){
      async.eachLimit(transactions, 10, function(transaction, txCb){
        var inputs = _.find(block.txs, function(tx) {return tx.rhash() === transaction.txid;}).inputs;
        transaction.spendCoins(inputs, txCb);
      }, cb);
    }
  ], function(err){
    if(err){
