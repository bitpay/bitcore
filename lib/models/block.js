...
BlockSchema.statics.addBlock = function(block, callback){
  ...
  async.series([
    ...
    function(cb){
      ...
      async.eachLimit(block.txs, 64, function(tx, txCb){
        async.eachOfLimit(tx.outputs, 64, function(output, index, outputCb){
        ...
        async.series([
          function(cb){
            async.parallel([
              function (cb) {
                Transaction.collection.insertMany(transactionInserts, { ordered: false }, cb);
              },
              function (cb) {
                Coin.collection.insertMany(coinInserts, { ordered: false }, cb);
              }
            ], cb);
          },
          function (cb) {
