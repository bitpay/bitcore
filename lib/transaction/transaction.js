...
Transaction._estimateFee = function(size, amountAvailable, feePerKb) {
  ...
  $.checkArgument(
    JSUtil.isNaturalNumber(size),
    'First argument "' + size + '" is expected to be the current estimate size in bytes'
  );
  $.checkArgument(
    JSUtil.isInteger(amountAvailable),
    'Second argument "' + amountAvailable + '" is expected to be amount available in satoshis'
  );
  $.checkArgument(
    JSUtil.isNaturalNumber(feePerKb) || _.isUndefined(feePerKb),
    'Third argument "' + feePerKb + '" is expected to be the fee per kilobyte in satoshis or undefined'
  );

  function calculateFee(sizeInBytes, satoshisPerKb) {
    return Math.round(sizeInBytes / 1000 * (satoshisPerKb || Transaction.FEE_PER_KB));
  }

  // Estimate fee with a change output
  var feeWithChangeOutput = calculateFee(size + Transaction.CHANGE_OUTPUT_MAX_SIZE, feePerKb);

  // Determine the amount expected in change
  var changeAmount = amountAvailable - feeWithChangeOutput;

  if (amountAvailable > feeWithChangeOutput && changeAmount > Transaction.DUST_AMOUNT) {
    // It's possible to include a change output and the resulting change
    // would be above the dust amount threshold, so we'll return the fee based on the
    // estimated size including a change output.
    return feeWithChangeOutput;
  } else {
    // Otherwise we'll estimate the fee without a change output (as doing such would lead
    // to a non-standard transaction). And we'll return the maximum between the amount
    // available and the fee estimated without the change output so that we will estimate
    // the fee including the dust remainder and not go below the minimum required fee.
    return Math.max(amountAvailable, calculateFee(size, feePerKb));
  }
