...
Transaction.prototype._estimateFee = function () {
  ...
  var available = this._getUnspentValue();
  var feeRate = this._feePerByte ? 
    (this._feePerByte || Transaction.FEE_PER_BYTE) : (this._feePerKb || Transaction.FEE_PER_KB) / 1000;
  return Math.ceil(Transaction._estimateFee(estimatedSize, available, feeRate));
};
...
Transaction._estimateFee = function(size, amountAvailable, feeRate) {
  function getFee() {
    return size * feeRate;
  }
  var noChangeFee = getFee();
  if (amountAvailable <= noChangeFee) {
    return noChangeFee;
  }
