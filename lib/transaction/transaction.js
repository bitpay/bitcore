 ...
 */
Transaction.prototype.from = function(utxo, pubkeys, threshold, script) {
  if (_.isArray(utxo)) {
    var self = this;
  ...
  if (pubkeys && threshold) {
    this._fromMultisigUtxo(utxo, pubkeys, threshold, script);
  } else {
...
Transaction.prototype._fromMultisigUtxo = function(utxo, pubkeys, threshold, script) {
  ...
  this.addInput(new MultiSigScriptHashInput({
    ...
    outputIndex: utxo.outputIndex,
    script: Script.empty()
  }, pubkeys, threshold, undefined, script));
};
