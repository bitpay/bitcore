...
Transaction.prototype.checkedSerialize = function() {
  var feeError = this._getFeeError();
...
Transaction.prototype._getFeeError = function() {
  var fee = this._getUnspentValue();
  var maximumFee = Math.ceil(Transaction.FEE_SECURITY_MARGIN * this._estimateFee());
  var minimumFee = Math.floor(this._estimateFee() / Transaction.FEE_SECURITY_MARGIN);
  if (fee > maximumFee) {
    return 'Fee is too high: expected at most ' + maximumFee + ' but found ' + fee;
  }
  if (fee < minimumFee) {
    return 'Fee is too low: expected at least ' + minimumFee + ' but found ' + fee;
  }
};
