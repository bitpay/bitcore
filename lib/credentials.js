...
Credentials.prototype._expand = function() {
  ...
  if (this.xPrivKey) {
    ...
    var addressDerivation = xPrivKey.derive(WalletUtils.PATHS.BASE_ADDRESS_DERIVATION[WalletUtils.DERIVATION_STRATEGIES.BIP45][this.network]);
...
Credentials.fromOldCopayWallet = function(w) {
  ...
  credentials.walletName = w.opts.name;
  credentials.walletId = w.opts.id;
  credentials.m = w.opts.requiredCopayers;
  credentials.n = w.opts.totalCopayers;
