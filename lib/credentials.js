 ...
 * so, xPubKey is PublicKeyHD(xPrivKey.deriveNonCompliantChild("m/44'/network'/account'").
 *
...
Credentials.prototype._expand = function() {
  ...
  if (this.xPrivKey) {
    ...
    var addressDerivation = xPrivKey.deriveNonCompliantChild(this.getBaseAddressDerivationPath());
    this.xPubKey = (new Bitcore.HDPublicKey(addressDerivation)).toString();

    var requestDerivation = xPrivKey.deriveNonCompliantChild(Constants.PATHS.REQUEST_KEY);
...
Credentials.prototype.getDerivedXPrivKey = function(password) {
  ...
  return new Bitcore.HDPrivateKey(xPrivKey, this.network).deriveNonCompliantChild(path);
};
  ...
  var pkr = _.map(w.publicKeyRing.copayersExtPubKeys, function(xPubStr) {
    ...
    if (isMe) {
      ...
      requestDerivation = (new Bitcore.HDPrivateKey(credentials.xPrivKey))
        .deriveNonCompliantChild(path).hdPublicKey;
    } else {
      // this 
