...
Credentials.keyFromMnemonic = function(words, passphrase, network) {
  var m = new Mnemonic(words);
  return m.toHDPrivateKey(passphrase, network).toString();
};

// note that mnemonic is NOT stored
Credentials.fromMnemonic = function(words, passphrase, network) {
  var key = Credentials.keyFromMnemonic(words, passphrase, network);
  return Credentials.fromExtendedPrivateKey(key);
};
...
Credentials.prototype._expand = function() {
  ...
  if (this.xPrivKey) {
    ...
    // TODO
    WalletUtils.PATHS.ENTROPY_SOURCE = WalletUtils.PATHS.ENTROPY_SOURCE || "m/2'";

    var auxKey = xPrivKey.derive(WalletUtils.PATHS.ENTROPY_SOURCE);
    var aux = (new Bitcore.HDPublicKey(auxKey)).publicKey;

    this.entropySource = Bitcore.crypto.Hash.ripemd160(Bitcore.crypto.Hash.sha256(aux.toBuffer())).toString('hex');
  } else {
    // Random priv key
