...
Credentials.fromExtendedPrivateKey = function(xPrivKey, account, derivationStrategy, opts) {
  ...
  x._expand(opts.nonCompliantDerivation);
  return x;
};
...
Credentials.prototype._expand = function(nonCompliantDerivation) {
  ...
  if (this.xPrivKey) {
    ...
    var deriveFn = !!nonCompliantDerivation ? _.bind(xPrivKey.deriveNonCompliantChild, xPrivKey) : _.bind(xPrivKey.deriveChild, xPrivKey);

    // this extra derivation is not to share a non hardened xPubKey to the server.
    var addressDerivation = deriveFn(this.getBaseAddressDerivationPath());
...
Credentials.prototype.getDerivedXPrivKey = function(password) {
  ...
  return new Bitcore.HDPrivateKey(xPrivKey, this.network).deriveChild(path);
};
