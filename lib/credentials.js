...
Credentials.fromMnemonic = function(network, words, passphrase, account, derivationStrategy, opts) {
  ...
  // entropySourcePath only used when importing mnemonics FROM a hardware wallet
  x._expand(opts.entropySourcePath);
  return x;
};
...
Credentials.prototype._expand = function(entropySourcePath) {
  ...
  if (entropySourcePath) {
    var seed = deriveFn(entropySourcePath).publicKey.toBuffer();
