...
Credentials.prototype.toSecret = function() {
  $.checkState(this.walletId);
  $.checkState(this.network);
  $.checkState(this.walletPrivKey);

  var walletPrivKey = Bitcore.PrivateKey.fromString(this.walletPrivKey).toWIF();
  var widHex = new Buffer(this.walletId.replace(/-/g, ''), 'hex');
  var widBase58 = new encoding.Base58(widHex).toString();
  return _.padRight(widBase58, 22, '0') + walletPrivKey + (this.network == 'testnet' ? 'T' : 'L');
};
