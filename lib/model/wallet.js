...
Wallet.create = function(opts) {
  ...
  x.beAuthPrivateKey = null; 
  x.beAuthPublicKey = null; 
...
Wallet.fromObj = function(obj) {
  ...
  x.beAuthPrivateKey = obj.beAuthPrivateKey; 
  x.beAuthPublicKey = obj.beAuthPublicKey; 
...
Wallet.prototype.updateBEKeys = function() {
  ...
  var salt = 'bws-auth-keysalt';

  var seed =_.map(this.copayers, 'xPubKey').join() + this.network + this.coin + salt;
  seed = bitcore.crypto.Hash.sha256(new Buffer(seed));
  var priv = bitcore.PrivateKey(seed, this.network);
  this.beAuthPrivateKey = priv.toString();
  this.beAuthPublicKey = priv.toPublicKey().toString();
};
...
Wallet.prototype.getSkippedAddress = function() {
  ...
  if (!next) return;
  log.verbose('Deriving addr:' + next.path);
