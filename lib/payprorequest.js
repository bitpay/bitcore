...
PayProRequest._nodeGet = function(opts, cb) {
  ...
  console.log('[payprorequest.js.7] NODE GET'); //TODO
  var match = opts.url.match(/^((http[s]?|ftp):\/)?\/?([^:\/\s]+)((\/\w+)*\/)([\w\-\.]+[^#?\s]+)(.*)?(#[\w\-]+)?$/);

  var opts = {
    agent: false,
    path: RegExp.$4 + RegExp.$6,
    host: RegExp.$3,
    proto: RegExp.$2,
  };
...
PayProRequest.get = function(opts, cb) {
  ...
  getter(opts, function(err, dataBuffer) {
    ...
    var amount = outputs[0].get('amount');
    amount = amount.low + amount.high * 0x100000000;


    var network = pd.get('network') == 'test' ? 'testnet' : 'livenet';
    var script = outputs[0].get('script').buffer;
    var addr = new Bitcore.Address.fromScript(new Bitcore.Script(script), network);
