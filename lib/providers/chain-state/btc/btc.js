...
BTCStateProvider.prototype.getBlocks = async function(network, sinceBlock, args) {
  let {limit} = args || {};
  ...
  let transformedBlocks = blocks.map((block) => Block._apiTransform(block, {object: true}));
...
BTCStateProvider.prototype.updateWallet = async function(
  ...
  return WalletAddress.updateCoins(
    { chain: this.chain, _id: walletId, network },
    addresses
  );
};
