...
var BIP32 = function(bytes) {
  var network = networks[bytes];
  if(network) {
    this.version = network.bip32privateVersion;
    this.depth = 0x00;
...
BIP32.prototype.initFromBytes = function(bytes) {
  ...
  for(var netname in networks) {
    ...
    isPrivate |= (this.version === network.bip32privateVersion);
    isPublic  |= (this.version === network.bip32publicVersion);
  }
...
BIP32.prototype.buildExtendedPublicKey = function() {
  ...
  for(var netname in networks) {
    var network = networks[netname];
    if(this.version === network.bip32publicVersion||
       this.version === network.bip32privateVersion) {
      v = network.bip32publicVersion;
      break;
    }
  }
  if(v == null) {
...
BIP32.prototype.deriveChild = function(i) {
  ...
  for(var netname in networks) {
    var network = networks[netname];
    isPrivate |= network.bip32privateVersion;
  }
