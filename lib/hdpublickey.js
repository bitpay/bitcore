 ...
 */
function HDPublicKey(arg, networkArg) {
  ...
  if (arg) {
    if (_.isString(arg) || bufferUtil.isBuffer(arg)) {
      ...
      // checks to see if it's an extended private key
      var network = Network.get(networkArg) || Network.defaultNetwork;
      var data = Base58Check.decode(arg);
      if (bufferUtil.integerFromBuffer(data.slice(0, 4)) === network.xprivkey) {
        return this._fromHDPrivateKey(arg, networkArg);
      }

      // otherwise handle as extendend public key
      var error = HDPublicKey.getSerializedError(arg, networkArg);
      if (!error) {
      ...
      if (_.isObject(arg)) {
        ...
        throw new TypeError('Invalid argument for creation, must be string, json, buffer, or object');
      }
    }
  } else {
...
HDPublicKey.prototype.derive = function (arg, hardened) {
  if (_.isNumber(arg)) {
    ...
    throw new TypeError('Invalid derivation argument');
  }
};

HDPublicKey.prototype._deriveWithNumber = function (index, hardened) {
  if (hardened || index >= HDPublicKey.Hardened) {
    throw new TypeError('Invalid index can not derive');
  }
...
HDPublicKey.getSerializedError = function (data, network) {
  ...
  if (!(_.isString(data) || bufferUtil.isBuffer(data))) {
    return new TypeError('Expected buffer or string');
  }
  if (!Base58.validCharacters(data)) {
    return new TypeError('Invalid Base58 character');
  }
  try {
  ...
  if (data.length !== HDPublicKey.DataSize) {
    return new TypeError('Invalid length for xpubkey');
  }
...
HDPublicKey._validateNetwork = function (data, networkArg) {
  ...
  if (!network) {
    return new TypeError('Expected network to be "livenet" or "testnet"');
  }
...
HDPublicKey.prototype._fromHDPrivateKey = function (arg, network) {
  var tmp = new HDPrivateKey(arg, network);
  return tmp.hdPublicKey;
};
