...
function Address(version, hash, currency) {
  if (!currency && hash && hash.length && (!Buffer.isBuffer(hash) || hash.length != 20))
    throw new Error('Hash must be 20 bytes ');
  Address.super_.call(this, version, hash, currency);
}
...
Address.fromPubKey = function(pubKey, network, currency) {
  if (!network)
    network = 'livenet';
  ...
  var version = networks[currency][network].addressVersion;
  var hash = coinUtil.sha256ripe160(pubKey);

  return new Address(version, hash, currency);
};

// create an address from a Key object
Address.fromKey = function(key, network, currency) {
  return Address.fromPubKey(key.public, network, currency);
};

// create a p2sh m-of-n multisig address
Address.fromPubKeys = function(mReq, pubKeys, network, opts, currency) {
  if (!network)
    network = 'livenet';
  ...
  return Address.fromScript(script, network, currency);
};

//create a p2sh address from redeemScript
Address.fromScript = function(script, network, currency) {
  if (!network)
    network = 'livenet';
  ...
  var version = networks[currency][network].P2SHVersion;
...
Address.fromScriptPubKey = function(scriptPubKey, network, currency) {
  ...
  if (payload) {
    ...
    switch (txType) {
      case Script.TX_PUBKEY:
        ...
        version = networks[currency][network].addressVersion;
        break;
      ...
      case Script.TX_SCRIPTHASH:
        version = networks[currency][network].P2SHVersion;
        break;
    }
    for (var i in payload)
...
Address.prototype.network = function() {
  ...
  var livenet = networks[currency].livenet;
  var testnet = networks[currency].testnet;
...
Address.fromPubkeyHashScriptSig = function(scriptSig, network, currency) {
  return Address.fromPubKey(scriptSig.chunks[1], network, currency);
};
...
Address.validate = function(s, currency) {
  return new Address(s, 'base58', currency || 'btc').isValid();
};
