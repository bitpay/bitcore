...
Credentials.createWithMnemonic = function(
  coin,
  network,
  passphrase,
  language,
...
Credentials.fromExtendedPrivateKey = function(
  coin,
  xPrivKey,
  account,
  derivationStrategy,
...
Credentials.fromMnemonic = function(
  coin,
  network,
  words,
  passphrase,
 ...
 */
Credentials.fromExtendedPublicKey = function(
  coin,
  xPubKey,
  source,
  entropySourceHex,
  ...
  $.checkArgument(
    entropyBuffer.length >= 14,
    'At least 112 bits of entropy are needed'
  );

  var x = new Credentials();
  x.coin = coin;
  x.xPubKey = xPubKey;
  x.entropySource = Bitcore.crypto.Hash.sha256sha256(entropyBuffer).toString(
    'hex'
  );
  x.account = account;
  x.derivationStrategy = derivationStrategy;
...
Credentials.prototype._expand = function() {
  ...
  */
  ...
  var network = Credentials._getNetworkFromExtendedKey(
  ...
  this.personalEncryptingKey = this._hashFromEntropy(
    'personalKey',
    16
  ).toString('base64');
...
Credentials.fromObj = function(obj) {
  ...
  x.derivationStrategy =
    x.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP45;
...
Credentials.prototype.addWalletInfo = function(
  walletId,
  walletName,
  m,
  n,
...
Credentials.fromOldCopayWallet = function(w) {
  function walletPrivKeyFromOldCopayWallet(w) {
    ...
    var privKey = new Bitcore.PrivateKey.fromBuffer(
  ...
  var pkr = _.map(w.publicKeyRing.copayersExtPubKeys, function(xPubStr) {
    ...
    if (isMe) {
      ...
      requestDerivation = new Bitcore.HDPrivateKey(
        credentials.xPrivKey
