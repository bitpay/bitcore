...
function decodeXAddress(address) {
  var xaddress = XAddress._decode(address);
  var network = xaddress.network;
  var hashBuffer = xaddress.hashBuffer;
  var script = Script.fromBuffer(hashBuffer);

  var info = {};
  if (script.isPublicKeyHashOut()) {
    info.type = Address.PayToPublicKeyHash;
    info.hashBuffer = script.chunks[2].buf;
  } else if (script.isScriptHashOut()) {
    info.type = Address.PayToScriptHash;
    info.hashBuffer = script.chunks[1].buf;
  }
...
Address._decodeXAddress = decodeXAddress
...
/**
 * Will return a xaddr representation of the address.
 * Can be converted by the caller to uppercase is needed (still valid).
 *
 * @returns {string} Bitcoin Cash address
 */
Address.prototype.toXAddress = function () {
  var script = Script.fromAddress(this);
  var xaddress = new XAddress(script.toBuffer(), this.network, this.type);
