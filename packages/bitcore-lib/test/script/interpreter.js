    ...
    for (let i = 0; i < script_asset_tests.length; i++) {
      ...
      it(`script asset test vector ${i}: ${test.comment}`, function() {
        ...
        if (test.success) {
          ...
          for (const flags of [testFlags]) { // allConsensusFlags() takes too long for the javascript implementation. Keeping the logic here for anyone who wants to run with allConsensusFlags
