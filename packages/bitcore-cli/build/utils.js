...
class Utils {
    ...
    static renderAmount(satoshis, coin, opts = {}) {
        if (!satoshis)
            return '0';
        return bitcore_wallet_client_1.Utils.formatAmount(satoshis, coin.toLowerCase(), { ...opts, fullPrecision: true }) + ' ' + coin.toUpperCase();
    }
    ...
    static async showMnemonic(walletName, mnemonic, opts) {
        ...
        fileText += 'MAKE SURE YOU WRITE DOWN THESE 12 WORDS AND SAVE THEM FOREVER' + os_1.default;
        fileText += 'If you lose these words, you will lose access to your wallet' + os_1.default.EOL;
        fileText += 'DO NOT SHARE THESE WORDS WITH ANYONE' + os_1.default.EOL;
    ...
    static amountFromSats(chain, sats) {
        chain = chain.toLowerCase();
        switch (chain) {
            case 'btc':
            case 'bch':
    ...
    static amountToSats(chain, amount) {
        chain = chain.toLowerCase();
        switch (chain) {
            case 'btc':
            case 'bch':
