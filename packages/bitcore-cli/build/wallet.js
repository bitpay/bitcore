...
class Wallet {
    ...
    static async getCurrencies(network = 'livenet') {
    ...
    async _signTxpTss(args) {
        ...
        const tssSign = new bitcore_wallet_client_1.TssSign.TssSign({
            baseUrl: url_1.default.resolve(this.host, '/bws/api'),
            credentials: __classPrivateFieldGet(this, _Wallet_walletData, "f").creds,
            tssKey: __classPrivateFieldGet(this, _Wallet_walletData, "f").key
        });
        try {
            const messageHash = crypto_wallet_core_1.ethers.keccak256(Client.getRawTx(txp)[0]).slice(2);
            await tssSign.start({
                id: txp.id,
