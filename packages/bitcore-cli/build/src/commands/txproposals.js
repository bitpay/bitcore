...
async function getTxProposals(args) {
    ...
    do {
        ...
        if (!txp) {
            ...
            lines.push(`Created: ${utils_1.Utils.formatDate(txp.createdOn * 1000)}`);
            ...
            if (txp.actions?.length) {
                ...
                lines.push(...txp.actions.map(a => {
                    return ` → ${a.copayerName}: ${a.type}${a.comment ? ` "${a.comment}"` : ''}${a.createdOn ? ` (${utils_1.Utils.formatDate(a.createdOn * 1000)})` : ''}`;
                }));
                lines.push('---------------------------');
            }
            if (txp.txid) {
