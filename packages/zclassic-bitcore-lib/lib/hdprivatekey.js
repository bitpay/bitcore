...
HDPrivateKey.fromSeed = function(hexa, network) {
  ...
  var privKeyBN = BN.fromBuffer(hash.slice(0, 32));
  // Loop until a valid secp256k1 scalar is found
  while (privKeyBN.cmp(BN.Zero) === 0 || privKeyBN.cmp(Point.getN()) >= 0) {
    console.warn('Invalid scalar, retrying:', privKeyBN.toString(16));
    hash = Hash.sha512hmac(hash, new buffer.Buffer('Bitcoin seed'));
    privKeyBN = BN.fromBuffer(hash.slice(0, 32));
  }
  console.warn('Generated private key:', privKeyBN.toString(16));
  return new HDPrivateKey({
    network: Network.get(network) || Network.defaultNetwork,
    depth: 0,
    parentFingerPrint: 0,
    childIndex: 0,
    privateKey: privKeyBN.toBuffer({size:32}),
