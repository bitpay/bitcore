...
describe.skip('Script', function() {
  // Skipped: Bitcoin test vectors
  ...
  describe('toAddress', function() {
    // Dati chiavi da testare
    var keys = [
      {
        pub: '03bcad742ab19c5db12840813200d06477090c10be60ad261cef7ba6732b2bbe59',
        address_mainnet: 't1WxGbWWctBeuvsm5XsThK3bKAkBua9xgxT',
        address_testnet: 'tmNo1vM12GrAR57xXCbmSAiG4mjGj7cVzju'
      }
    ];

    keys.forEach(function(k, index) {
	  var pubkey = new PublicKey(k.pub);
      var liveAddress = pubkey.toAddress(Networks.livenet);
      var testAddress = pubkey.toAddress(Networks.testnet);
	  

      it('should generate correct mainnet address for key #' + (index+1), function() {
        liveAddress.toString().should.equal(k.address_mainnet);
        liveAddress.network.should.equal(Networks.livenet);
      });

      it('should generate correct testnet address for key #' + (index+1), function() {
        testAddress.toString().should.equal(k.address_testnet);
        testAddress.network.should.equal(Networks.testnet);
      });

      it('should prioritize network argument in script for key #' + (index+1), function() {
        var script = new Script(liveAddress);
        script.toAddress(Networks.testnet).toString().should.equal(k.address_testnet);
        script.toAddress(Networks.testnet).network.should.equal(Networks.testnet);
      });

      it('should use inherited network in script for key #' + (index+1), function() {
        var script = new Script(liveAddress);
        script.toAddress().toString().should.equal(k.address_mainnet);
        script = new Script(testAddress);
        script.toAddress().toString().should.equal(k.address_testnet);
      });
    });

    // Puoi aggiungere altri test generici come l'originale, se serve.
  });
