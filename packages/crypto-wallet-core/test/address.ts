...
describe('Address Derivation', () => {
  ...
  const rootXPriv =
    'xprv9s21ZrQH143K2JcLRF8F1SkpXE9wop7gmbaifTPVNYzySCkfLg45yWT7nzicN3BBHCrsA1KoLSoxdnFJJDiqMnwGETdkLRrjZ7QPQ8NdERA';

  const rootXPub =
    'xpub661MyMwAqRbcEngoXGfFNahZ5FzSDGqY8pWKTqo6vtXxK15otDNLXJmbeHV7DUjvPc7CAFhYp6hzBiTanr8rgoHPHf6NSgZAyejK5bk8MiW';

  describe('ETH', () => {
    it('should be able to generate a valid ETH address', () => {
      const xPub =
        'xpub6D8rChqkgFuaZULuq2n6VrS4zB5Cmv24gcRc889dFRRgYAH1CGQmQZ9kcPfMAfWGPnyMd1X5foBYFmJ5ZPfvwhm6tXjaY13ao1rQHRtkKDv';
      // 'select scout crash enforce riot rival spring whale hollow radar rule sentence';
    ...
    it('should be able to generate a valid ETH address, privKey, pubKey', () => {
      const privKey =
        'xprv9ypBjKErGMqCdzd44hfSdy1Vk6PGtU3si8ogZcow7rA23HTxMi9XfT99EKmiNdLMr9BAZ9S8ZKCYfN1eCmzYSmXYHje1jnYQseV1VJDDfdS';

      const path = Deriver.pathFor('ETH', 'mainnet');
      expect(path).to.equal("m/44'/60'/0'");

      const result = Deriver.derivePrivateKey(
        'ETH',
        'mainnet',
        privKey,
        0,
      ...
      const expectedResult = {
        ...
        privKey:
          '62b8311c71f355c5c07f6bffe9b1ae60aa20d90e2e2ec93ec11b6014b2ae6340',
        pubKey:
          '0386d153aad9395924631dbc78fa560107123a759eaa3e105958248c60cd4472ad'
      };
  ...
  describe('XRP', () => {
    it('should be able to generate a valid XRP address', () => {
      const xPub =
        'xpub6J8BBe8QHMMiVQK1F8hLpRKnmkwNTa7tkg753KWjkafzcxfWVFBMkpbPjfY9Fz4bgSvn6jiUYg1ivpeF5HjE6jvrdHm6Se7HKgEAjPFGFfr';
      // 'select scout crash enforce riot rival spring whale hollow radar rule sentence';
    ...
    it('should be able to generate a valid XRP address, privKey, pubKey', () => {
      const privKey =
        'xprvA58pn8bWSyoRGvEY97ALTHP4Dj6t47Q3PTBUEw78CF91kALMwhs7D2GutQSvpRN6ACR4RX4HbF3KmF7zDf48gR8nwG7DqLp6ezUcMiPHDtV';
  ...
  describe('LTC', () => {
    it('should be able to generate a valid LTC address', () => {
      ...
      const ltcKey = Libs.BTC.HDPrivateKey(rootXPriv).derive(path);
      const xPub = ltcKey.xpubkey;

      const address = Deriver.deriveAddress('LTC', 'livenet', xPub, 0, false);
      const expectedAddress = 'LeQrWcDs3nEQfnRKFYkDcntGrFmnASM15E';
      expect(address).to.equal(expectedAddress);
    });

    it('should be able to generate a valid LTC address, privKey, pubKey', () => {
      const path = Deriver.pathFor('LTC', 'mainnet');
      expect(path).to.equal("m/44'/2'/0'");

      const ltcKey = Libs.BTC.HDPrivateKey(rootXPriv).derive(path);
      const privKey = ltcKey.toString();
      ...
      const expectedResult = {
        ...
        privKey: 'f860106f60e3f520e31e3a5b6e0e155391277eeb8cbd24370b26f7e9a7e19c94',
        pubKey:
