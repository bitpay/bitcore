...
export class ZclMessage implements IMessageClass {
  getMessageHash(args: { message: string; encoding?: Encoding }): Buffer | string {
    const { message, encoding } = args;
    const buf = Buffer.from(new ZclassicBitcoreLib.Message(message).magicHash());
  ...
  signMessage(args: { message: string; privateKey: KeyType; encoding?: Encoding }): ISignedMessage {
    const { message, encoding } = args;
    let { privateKey } = args;
    privateKey = new ZclassicBitcoreLib.PrivateKey(privateKey);
    const buf = Buffer.from(new ZclassicBitcoreLib.Message(message).sign(privateKey), 'base64');
    return {
  ...
  signMessageWithPath(args: { message: string; hdPrivateKey: HDKeyType; derivationPath: string; encoding?: Encoding }): ISignedMessage {
    ...
    const privateKey = new ZclassicBitcoreLib.HDPrivateKey(hdPrivateKey).deriveChild(derivationPath).privateKey;
