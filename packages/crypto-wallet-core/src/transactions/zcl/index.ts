...
import ZclassicBitcoreLib from 'zclassic-bitcore-lib';
import type { Key } from '../../types/derivation';

const $ = ZclassicBitcoreLib.util.preconditions;
...
export class BTCTxProvider {
  ...
  applySignature(params: { tx: ZclassicBitcoreLib.Transaction; signature: SignatureType; index: number; sigtype?: number }) {
  ...
  getSighash(params: {
    tx: string | ZclassicBitcoreLib.Transaction;
    index: number;
    utxos?: ZclassicBitcoreLib.Transaction.UnspentOutput[];
    pubKey?: string | ZclassicBitcoreLib.PublicKey | ZclassicBitcoreLib.HDPublicKey;
    path?: string;
    sigtype?: number;
    // Multisig params for `associateInputs()`
    /** Multisig public keys */
    pubKeys?: string[] | ZclassicBitcoreLib.PublicKey[];
...
type SignatureType = ZclassicBitcoreLib.Transaction.Signature | ZclassicBitcoreLib.crypto.Signature | TssSig;
