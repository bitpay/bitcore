      ...
      [
        next => {
          this.getWallet({}, (err, wallet) => {
            ...
            status.serverMessage = deprecatedServerMessage(
              wallet,
              this.appName,
              this.appVersion
            );
      ...
      this.syncWallet(wallet, err => {
        ...
        this._getUtxosForCurrentWallet(
          ...
          async (err, utxos) => {
            ...
            const byAddress = {};
            _.each(
              _.keyBy(_.sortBy(utxos, 'address'), 'address'),
              (value, key) => {
                byAddress[key] = {
                  address: key,
                  path: value.path,
            ...
            if (wallet.coin === 'eth') {
              ...
              Object.assign(balance, {
                totalAmount:
                  balanceResponse.balance / 1e10 || balance.totalAmount,
                lockedAmount:
                  balanceResponse.unconfirmed / 1e10 || balance.lockedAmount,
                totalConfirmedAmount:
                  balanceResponse.confirmed / 1e10 ||
                  balance.totalConfirmedAmount,
                lockedConfirmedAmount:
                  balanceResponse.unconfirmed / 1e10 ||
                  balance.totalConfirmedAmount,
                availableAmount:
                  balanceResponse.confirmed / 1e10 || balance.availableAmount,
  ...
  getFeeLevels(opts, cb) {
    ...
    this.storage.checkAndUseGlobalCache(
      ...
      (err, values, oldvalues) => {
        ...
        this._sampleFeeLevels(
          opts.coin,
          opts.network,
          samplePoints(),
          (err, feeSamples, failed) => {
            if (err) {
              if (oldvalues) {
                log.warn(
    ...
    async.series(
      [
            ...
            this._normalizeTxHistory(
              ...
              (err, inTxs: any[]) => {
                ...
                if (cacheStatus.tipTxId) {
                  ...
                  //
                  log.info(
                    `Storing stream cache for ${wallet.id}: ${
                      lastTxs.length
                    } txs`
                  );
        ...
        next => {
          ...
          const updateHeight =
            bcHeight - Defaults.CONFIRMATIONS_TO_START_CACHING;
