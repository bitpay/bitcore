...
export class RskChain implements IChain {
  /**
  ...
  private convertBitcoreBalance(bitcoreBalance, locked) {
    ...
    // we ASUME all locked as confirmed, for RSK.
  ...
  getWalletBalance(server, wallet, opts, cb) {
    ...
    bc.getBalance(wallet, (err, balance) => {
      ...
      server.getPendingTxs(opts, (err, txps) => {
        ...
        // Do not lock rsk multisig amount
  ...
  getBitcoreTx(txp, opts = { signed: true }) {
    ...
    const chain = isETHMULTISIG ? 'ETHMULTISIG' : isERC20 ? 'ERC20' : 'RSK';
        ...
        */
        ...
        if (totalAmount < txpTotalAmount) {
          ...
          if (opts.tokenAddress || opts.multisigContractAddress) {
            // RSK linked wallet balance
            server.getBalance({}, (err, rskBalance) => {
              if (err) return cb(err);
              const { totalAmount, availableAmount } = rskBalance;
