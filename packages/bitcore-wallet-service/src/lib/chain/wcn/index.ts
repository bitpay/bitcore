...
import { BitcoreLibWcn } from 'crypto-wallet-core';
...
export class WcnChain extends BtcChain implements IChain {
  constructor(private bitcoreLibWcn = BitcoreLibWcn) {
    super(BitcoreLibWcn);
  }

  selectTxInputs(server, txp, wallet, opts, cb) {
    const MAX_TX_SIZE_IN_KB = Defaults.MAX_TX_SIZE_IN_KB_WCN;
    ...
    const select = (utxos, coin, cb) => {
      ...
      _.each(smallInputs, (input, i) => {
        ...
        if (netTotal >= txpAmount) {
          ...
          const dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, this.bitcoreLibWcn.Transaction.DUST_AMOUNT);
