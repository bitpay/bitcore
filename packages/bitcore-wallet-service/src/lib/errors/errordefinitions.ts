...
interface Errors<T> {
  ...
  TSS_SESSION_EXPIRED: T;
  TSS_MISMATCH_VERSION: T;
  TSS_FINAL_SIGNATURE_MISMATCH: T;
};
...
const errors: Errors<string> = {
  ...
  TSS_SESSION_EXPIRED: 'TSS session has expired',
  TSS_MISMATCH_VERSION: 'TSS version does not match session version',
  TSS_FINAL_SIGNATURE_MISMATCH: 'Final signature does not match the one generated by the session',
};
