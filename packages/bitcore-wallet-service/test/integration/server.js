...
describe('Wallet service', function() {
    ...
    it('should correctly get send max info when resulting amount is below dust', function(done) {
      helpers.stubUtxos(server, wallet, [300e-6, 300e-6], function() {
        server.getSendMaxInfo({
          ...
          should.not.exist(err);
          should.exist(info);
          info.size.should.equal(642);
          info.fee.should.equal(32100);
          info.amount.should.equal(27900);
  ...
  describe('#getSendMaxInfo PAYPRO', function() {
    var server, wallet;
    beforeEach(function(done) {
      helpers.createAndJoinWallet(2, 3, function(s, w) {
        server = s;
