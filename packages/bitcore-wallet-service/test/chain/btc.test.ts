...
describe('Chain BTC', function() { 
  ...
  describe('#getEstimatedSize', function() {
    let btc;
    let fromAddress;
    let witnessFromAddress;
    let simpleUtxoWith1BTC;
    let simpleWitnessUtxoWith1BTC;
    let changeAddress;
    let toAddress;
    let privateKey;

    before(function() {
      btc = ChainService.get('btc') as BtcChain;
  ...
  describe('#selectTxInputs', function() {
    ...
    beforeEach(async function() {
      ...
      ({
        wallet,
        server
      } = await helpers.createAndJoinWallet(2, 3, {}));
      utxos = await helpers.stubUtxos(server, wallet, [1, 2, 3, 4, 5], {});
    });
