...
const EscrowInput = require('../../lib/transaction/input/escrow');

const BN = bitcore.crypto.BN;
const Transaction = bitcore.Transaction;
const Signature = bitcore.Signature;
...
describe('Transaction', function() {
  ...
  describe.only('getSighash', function() {
    it('should get sighash for publickey input', function() {
      ...
      sighash.toString('hex').should.equal('3b751dcecca7be98256ba00f1c1d41af33a67e2a0d0a54c280eb69d44c9abad6');
    ...
    it('should get sighash for publickeyhash input', function() {
      ...
      const sighash = t.inputs[0].getSighash(t, p2shPublicKey1, 0);
      should.exist(sighash);
      sighash.toString('hex').should.equal('f8363382d661497183a867bfc0602885b968506065dec350fba901533ef01d26');
      const manualSig = ECDSA.sign(sighash, p2shPrivateKey1); // No { endian: 'little' } since getSighash already does a reverse()
      const autoSig = t.inputs[0].getSignatures(t, p2shPrivateKey1, 0)[0];
      manualSig.toString('hex').should.equal(autoSig.signature.toString());
    });
    ...
    it('should get sighash for multisig input', function() {
      ...
      const sighash = t.inputs[0].getSighash(t, p2shPublicKey1, 0);
      should.exist(sighash);
      sighash.toString('hex').should.equal('9b712a032091e7bf6e98959b4675ad96a05c1361632f829431427d9a1a4111e9');
      const manualSig = ECDSA.sign(sighash, p2shPrivateKey1); // No { endian: 'little' } since getSighash already does a reverse()
      const autoSig = t.inputs[0].getSignatures(t, p2shPrivateKey1, 0)[0];
      manualSig.toString('hex').should.equal(autoSig.signature.toString());
    });
    ...
    it('should get sighash for multisigscripthash input', function() {
      ...
      const sighash = t.inputs[0].getSighash(t, p2shPublicKey1, 0);
      should.exist(sighash);
      sighash.toString('hex').should.equal('362a2327c6b9e10557d3a023db7e49fdcc29ee4500b40f610d89d1a1624ed22e');
      const manualSig = ECDSA.sign(sighash, p2shPrivateKey1); // No { endian: 'little' } since getSighash already does a reverse()
      const autoSig = t.inputs[0].getSignatures(t, p2shPrivateKey1, 0)[0];
      manualSig.toString('hex').should.equal(autoSig.signature.toString());
    });
...
const tx_empty_hex = '02000000000000000000';
