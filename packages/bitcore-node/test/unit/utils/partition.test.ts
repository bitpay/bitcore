...
describe('Partition', () => {
  ...
  it('should split an array of 5 to 5 arrays', () => {
    ...
    const partitioned = partition(testArr, 5);
  ...
  it('should handle 0', () => {
    const testArr = [1, 2, 3, 4, 5];
    const partitioned = partition(testArr, 0);
    expect(partitioned).to.deep.equal([[1], [2], [3], [4], [5]]);
    expect(testArr).to.deep.equal([1, 2, 3, 4, 5]);
  });
  ...
  it('should handle one', () => {
    const testArr = [1, 2, 3, 4, 5];
    const partitioned = partition(testArr, 1);
    expect(partitioned).to.deep.equal([[1], [2], [3], [4], [5]]);
    expect(testArr).to.deep.equal([1, 2, 3, 4, 5]);
  });
  ...
  it('should handle two', () => {
    const testArr = [1, 2, 3, 4, 5];
    const partitioned = partition(testArr, 2);
    expect(partitioned).to.deep.equal([[1, 2], [3, 4], [5]]);
    expect(testArr).to.deep.equal([1, 2, 3, 4, 5]);
  });
  ...
  it('should handle between one and zero', () => {
    const testArr = [1, 2, 3, 4, 5];
    const partitioned = partition(testArr, 0.15);
    expect(partitioned).to.deep.equal([[1], [2], [3], [4], [5]]);
    expect(testArr).to.deep.equal([1, 2, 3, 4, 5]);
  });
  ...
  it('should handle different sizes of arrays', () => {
    for (let i = 0; i < 1000; i++) {
      ...
      let adjustment = 0;
      const randomArr = new Array(randomLen).fill(1).map(num => num + adjustment++);
      const partitioned = partition(randomArr, 3);
      ...
      const lastBatch = partitioned[partitioned.length - 1];
      if (!lastBatch) {
