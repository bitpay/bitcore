...
class Migration {
  ...
  processArgs(argv) {
    ...
    if (helpIdx >= 0) {
      console.log("Usage: node migration-2.js --chain [CHAIN] --network [NETWORK] --dryrun [BOOL - default: true]");
      this.endProcess();
    }
  ...
  async runScript(args) {
    ...
    const filename = `migration-2-output-${chain}-${network}-${date}.log`;
    ...
    if (actuallySpent[`${chain}-${network}`]) {
      ...
      const date = new Date().getTime();
      const filename = `migration-2-actually-spent-${chain}-${network}-${date}.log`;
