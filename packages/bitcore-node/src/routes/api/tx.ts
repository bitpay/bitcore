...
router.get('/', function(req, res) {
  ...
  return InternalState.streamTransactions(payload);
});
...
router.get('/:txId', function(req, res) {
  ...
  chain = chain.toUpperCase();
  network = network.toLowerCase();
  return InternalState.streamTransaction({ chain, network, txId, stream: res });
});
...
router.get('/:txid/coins', (req, res, next) => {
  ...
  if (typeof txid !== 'string' || typeof chain !== 'string' || typeof network !== 'string') {
    ...
    chain = chain.toUpperCase();
    network = network.toLowerCase();
    InternalState.getCoinsForTx({ chain, network, txid })
      .then(coins => {
...
router.post('/send', async function(req, res) {
  try {
    ...
    chain = chain.toUpperCase();
    network = network.toLowerCase();
    let txid = await InternalState.broadcastTransaction({
      chain,
      network,
      rawTx
    });
