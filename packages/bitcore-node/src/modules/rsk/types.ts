...
export interface RskBlock {
  header: RskHeader;
  transactions: Transaction[];
  uncleHeaders: RskHeader[];
  raw: Buffer[];
  txTrie: any;
}

export interface RskHeader {
  parentHash: Buffer;
  uncleHash: Buffer;
  coinbase: Buffer;
  stateRoot: Buffer;
...
export interface Transaction {
  ...
  value: Buffer;
  data: Buffer;
  // EIP 155 chainId - mainnet: 30, testnet: 31
  chainId: number;
  getUpfrontCost: () => BN;
}

export type IRskBlock = IBlock & {
  coinbase: Buffer;
  nonce: Buffer;
  gasLimit: number;
  gasUsed: number;
...
export type IRskTransaction = ITransaction & {
  data: Buffer;
  gasLimit: number;
  gasPrice: number;
  nonce: number;
...
export interface RskTransactionJSON {
  txid: string;
  chain: string;
  network: string;
  blockHeight: number;
  ...
  to: string;
  from: string;
  abiType?: IRskTransaction['abiType'];
  decodedData?: AbiDecodedData;
  data: string;
  internal: Array<DecodedTrace>;
  receipt?: IRskTransaction['receipt'];
}
