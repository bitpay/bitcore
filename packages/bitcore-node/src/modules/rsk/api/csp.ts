...
export class RSKStateProvider extends InternalStateProvider implements IChainStateService {
  config: any;
  ...
  async getWeb3(network: string): Promise<{ rpc: CryptoRpc; web3: Web3 }> {
    ...
    return RSKStateProvider.rpcs[network];
  }
  ...
  async getERC20TokenInfo(network: string, tokenAddress: string) {
    const token = await RSK.erc20For(network, tokenAddress);
  ...
  async getFee(params) {
    ...
    return CacheStorage.getGlobalOrRefresh(
      cacheKey,
      async () => {
        const txs = await RskTransactionStorage.collection
  ...
  async getLocalTip({ chain, network }) {
    return RskBlockStorage.getLocalTip({ chain, network });
  }
  ...
  async populateReceipt(tx: MongoBound<IRskTransaction>) {
    if (!tx.receipt) {
      const receipt = await this.getReceipt(tx.network, tx.txid);
      if (receipt) {
        const fee = receipt.gasUsed * tx.gasPrice;
        await RskTransactionStorage.collection.updateOne({ _id: tx._id }, { $set: { receipt, fee } });
        tx.receipt = receipt;
        tx.fee = fee;
      }
    }
  ...
  async getTransaction(params: StreamTransactionParams) {
    try {
      ...
      let found = await RskTransactionStorage.collection.findOne(query);
      if (found) {
        let confirmations = 0;
        ...
        const convertedTx = RskTransactionStorage._apiTransform(found, { object: true }) as RskTransactionJSON;
  ...
  async streamAddressTransactions(params: StreamAddressUtxosParams) {
    ...
    if (!args.tokenAddress) {
      ...
      Storage.apiStreamingFind(RskTransactionStorage, query, { limit /*since, paging: '_id'*/ }, req!, res!);
    } else {
      try {
  ...
  async streamTransactions(params: StreamTransactionsParams) {
    ...
    const tip = await this.getLocalTip(params);
    const tipHeight = tip ? tip.height : 0;
    return Storage.apiStreamingFind(RskTransactionStorage, query, args, req, res, t => {
      let confirmations = 0;
      if (t.blockHeight !== undefined && t.blockHeight >= 0) {
        confirmations = tipHeight - t.blockHeight + 1;
      }
      const convertedTx = RskTransactionStorage._apiTransform(t, { object: true }) as Partial<ITransaction>;
  ...
  async streamWalletTransactions(params: StreamWalletTransactionsParams) {
    ...
    const { web3 } = await this.getWeb3(network);
    const query = RSK.getWalletTransactionQuery(params);
    ...
    const rskTransactionTransform = new RskListTransactionsStream(walletAddresses);
    const populateReceipt = new PopulateReceiptTransform();

    transactionStream = RskTransactionStorage.collection
      .find(query)
  ...
  async estimateGas(params): Promise<number> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const { web3 } = await this.getWeb3(network);
        const dataDecoded = RskTransactionStorage.abiDecode(data);
  ...
  async getBlocks(params: GetBlockParams) {
    ...
    let cursor = RskBlockStorage.collection.find(query, options).addCursorFlag('noCursorTimeout', true);
    if (options.sort) {
    ...
    const tip = await this.getLocalTip(params);
    const tipHeight = tip ? tip.height : 0;
    const blockTransform = (b: IRskBlock) => {
      let confirmations = 0;
      if (b.height && b.height >= 0) {
        confirmations = tipHeight - b.height + 1;
      }
      const convertedBlock = RskBlockStorage._apiTransform(b, { object: true }) as IRskBlock;
  ...
  async updateWallet(params: UpdateWalletParams) {
    ...
    for (let addressBatch of addressBatches) {
      ...
      await RskTransactionStorage.collection.updateMany(
        {
          $or: [
