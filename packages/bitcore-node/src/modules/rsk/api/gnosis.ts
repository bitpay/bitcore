...
export class GnosisApi {
  ...
  async multisigFor(network: string, address: string) {
    const { web3 } = await RSK.getWeb3(network);
  ...
  async getMultisigContractInstantiationInfo(
    ...
    const { web3 } = await RSK.getWeb3(network);
    const networkConfig = Config.chainConfig({ chain: 'RSK', network });
    const { gnosisFactory = this.gnosisFactories[network] } = networkConfig;
    let query = { chain: 'RSK', network, txid: txId };
    const found = await RskTransactionStorage.collection.findOne(query);
  ...
  async getMultisigTxpsInfo(network: string, multisigContractAddress: string): Promise<Partial<Transaction>[]> {
    ...
    const [block] = await RskBlockStorage.collection
      .find({
        chain: 'RSK',
        network,
  ...
  async streamGnosisWalletTransactions(params: { multisigContractAddress: string } & StreamWalletTransactionsParams) {
    ...
    const { web3 } = await RSK.getWeb3(network);
    const transactionQuery = RSK.getWalletTransactionQuery(params);
    ...
    const rskTransactionTransform = new RskListTransactionsStream([multisigContractAddress, args.tokenAddress]);
    ...
    if (multisigContractAddress) {
      const rskMultisigTransform = new EthMultisigRelatedFilterTransform(
        web3,
        multisigContractAddress,
        args.tokenAddress
      );
      transactionStream = transactionStream.pipe(rskMultisigTransform);
    }
