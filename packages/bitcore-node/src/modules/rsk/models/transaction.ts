...
@LoggifyClass
export class RskTransactionModel extends BaseTransaction<IRskTransaction> {
  ...
  async batchImport(params: {
    ...
    if (params.height < SpentHeightIndicators.minimum) {
      for (let op of txOps) {
        ...
        const tx = { ...op.updateOne.update.$set, ...filter } as IRskTransaction;
  ...
  async addTransactions(params: {
    ...
    if (parentChain && forkHeight && height < forkHeight) {
      const parentTxs = await RskTransactionStorage.collection
      ...
      return Promise.all(
        params.txs.map(async (tx: IRskTransaction) => {
  ...
  getTransactions(params: { query: any; options: StreamingFindOptions<IRskTransaction> }) {
  ...
  _apiTransform(
    tx: IRskTransaction | Partial<MongoBound<IRskTransaction>>,
    options?: TransformOptions
  ): RskTransactionJSON | string {
    const dataStr = `0x${tx.data!.toString('hex')}`;
    const decodedData = this.abiDecode(dataStr);

    const transaction: RskTransactionJSON = {
      txid: tx.txid || '',
      network: tx.network || '',
      chain: tx.chain || '',
...
export let RskTransactionStorage = new RskTransactionModel();
