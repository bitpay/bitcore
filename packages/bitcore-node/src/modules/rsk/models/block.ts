...
import { IRskBlock, IRskTransaction } from '../types';
import { RskTransactionStorage } from './transaction';

@LoggifyClass
export class RskBlockModel extends BaseBlock<IRskBlock> {
  ...
  async processBlock(params: {
    ...
    await RskTransactionStorage.batchImport({
      txs: transactions,
  ...
  async getBlockOp(params: { block: IRskBlock; chain: string; network: string }) {
    const { block, chain, network } = params;
  ...
  _apiTransform(block: Partial<MongoBound<IRskBlock>>, options?: TransformOptions): any {
    const transform = {
      _id: block._id,
      chain: block.chain,
...
export let RskBlockStorage = new RskBlockModel();
