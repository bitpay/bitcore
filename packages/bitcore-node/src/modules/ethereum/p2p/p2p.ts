...
export class EthP2pWorker extends BaseP2PWorker<IEthBlock> {
           ...
           try {
             ...
             while (currentHeight <= bestBlock) {
               ...
               if (now - lastLog > oneSecond) {
                 ...
                 logger.info(
                   `${timestamp()} | Syncing... | Chain: ${chain} | Network: ${network} |${(
                     blocksProcessed / elapsedMinutes
                   )
                     .toFixed(2)
         ...
         convertTx(tx: Partial<ParityTransaction>, block?: IEthBlock): IEthTransaction {
           ...
           if (block) {
             ...
             convertedTx.blockTime = blockTime;
             convertedTx.blockTimeNormalized = blockTimeNormalized;
           }
           return convertedTx;
         }
